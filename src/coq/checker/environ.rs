...
/// initialized version freely at lifetime 'b, without making us use separate maps or something
...
enum LazyRef<'b, T> where T: 'b {
    Owned(&'b mut T),
    Borrowed(&'b T),
}

impl<'b, T> AsRef<T> for LazyRef<'b, T> {
    fn as_ref(&self) -> &T {
        match *self {
...
pub struct Globals<'b> {
    ...
    constants: CMapEnv<LazyRef<'b, Cb>>,
...
impl<'b, 'g> Env<'b, 'g> {
    ...
    pub fn lookup_constant(&self, c: &Cst) -> Option<&Cb> {
        self.globals.constants.get(c).map( |c| c.as_ref() )
    }

    fn lookup_constant_mut(&mut self, c: &Cst) -> Option<&mut LazyRef<'g, Cb>> {
        self.globals.constants.get_mut(c)
    }

    pub fn constant_value(&mut self, o: &PUniverses<Cst>) ->
        Option<Result<&'b Constr, ConstEvaluationResult>> {
        ...
        self.lookup_constant_mut(kn)
            .and_then( |rf| {
                ...
                // The method involves storing a dummy &'b Cb in the Env structure and would
